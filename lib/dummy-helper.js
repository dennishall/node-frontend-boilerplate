//todo: refactor variable names so they are less confusing
//      (reloadCss doesn't sound like a timestamp, etc.;).
//      function name "reload" feels overused.
...
module.exports = function(app, timestamps) {
    ...
    app.post('/reload-content/', function(req, res) {
        console.log('/reload-content/  request: ');
        console.log(req);
    ...
    function getValidationSource(filePath, source, res, cacheKey) {
        // TODO: rewrite with node http-agent instead!
        // todo: (interim) - use w3c validator, not *mape's*
    ...
    */
    ...
    app.get(/.*/, function(req, res) {
        ...
        function render(overlay) {
            ...
            res.render(file, {
                locals: {
                    'page': req.url //(urlMatch) ? '/'+urlMatch+'/' : '/'
                    , 'date': new Date().toString()
                    , 'dummyHelperHtml': overlay ? '<div id="dummy-overlay"><div id="dummy-overlay-bg" style="background-image: url(/public/img/overlays/'+overlay+');"></div></div>' : ''
                    , port : app.settings.port
                    , googleAnalyticsSiteId: app.settings.googleAnalyticsSiteId
                }
            });
        }
    });
